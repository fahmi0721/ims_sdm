...
function getPendidikanFormal($Kode){
    $r = $GLOBALS['db']->query("SELECT Nama FROM ims_master_pendidikan_formal WHERE Kode = '$Kode'")->fetch(PDO::FETCH_ASSOC);
    return strtoupper("Pendidikan Formal : ".$r['Nama']);
}

function getJurusan($Kode){
    $r = $GLOBALS['db']->query("SELECT Nama FROM ims_master_sub_pendidikan_formal WHERE Kode = '$Kode'")->fetch(PDO::FETCH_ASSOC);
    return strtoupper("Jurusan : ".$r['Nama']);
}

function getPendidikanNonFormal($Kode){
    $r = $GLOBALS['db']->query("SELECT Nama FROM ims_master_pendidikan_nonformal WHERE Kode = '$Kode'")->fetch(PDO::FETCH_ASSOC);
    return strtoupper("Pendidikan Non Formal : ".$r['Nama']);
}

function getNamaPendidikan($data){
    $Nama = array();
    if(!empty($data)){
        if(!empty($data['b.KodeMaster'])){
            $Nama[]= getPendidikanFormal($data['b.KodeMaster']);
        }

        if(!empty($data['b.KodeSubMaster'])){
            $Nama[]= getJurusan($data['b.KodeSubMaster']);
        }
...
$_GET['b.KodeMaster'] = $_GET['KodeFormal'];
$_GET['b.KodeSubMaster'] = $_GET['KodeJurusan'];
unset($_GET['aksi']);
unset($_GET['Id']);
unset($_GET['KodeFormal']);
unset($_GET['KodeJurusan']);
$NamaPendidikan = getNamaPendidikan($_GET);
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'REKAP DATA PERCABANG BERDASARKAN PENDIDIKAN '.$NamaPendidikan)
            ->setCellValue('A3', 'NO')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle("Rekap Data Pendidikan");
...
header('Content-Disposition: attachment;filename="'.$Time.'-Rekap-Data-Percabang-Berdasarkan-Pendidikan.xls"');
