...
function getNamaPendidikan($data){
    ...
    if(!empty($data)){
        if(!empty($data['b.KodeMaster'])){
            $Nama[]= getPendidikan($data['b.KodeMaster']);
        }
        return implode(",", $Nama);
    }else{
        return "SEMUA AGAMA";
...
function RekapData($data){
    ...
    $sql = "SELECT COUNT(a.Id) as Total, a.KodeCabang, a.SpkPengangkatan FROM ims_master_biodata a LEFT JOIN ims_pendidikan_nonformal b ON a.NoKtp = b.NoKtp  WHERE a.Flag = '1' $Filter  GROUP BY a.KodeCabang ORDER BY a.KodeCabang ASC";
...
$objPHPExcel->getProperties()->setCreator("PT ISMA")
							 ...
							 ->setTitle("Rekap Data Sertifkasi")
							 ->setSubject("Rekap Data Sertifkasi")
							 ->setDescription("Rekap Data Sertifkasi")
							 ->setKeywords("Rekap Data Sertifkasi")
							 ->setCategory("Rekap Data Sertifkasi");
...
$_GET['b.KodeMaster'] = $_GET['Kode'];
unset($_GET['aksi']);
unset($_GET['Id']);
unset($_GET['Kode']);
$NamaPendidikan = getNamaPendidikan($_GET);
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'REKAP DATA PERCABANG BERDASARKAN  '.$NamaPendidikan)
            ->setCellValue('A3', 'NO')
