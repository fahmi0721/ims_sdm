...
function FilterData($data){
    ...
    if(!empty($data['MasaKerja'])){
        if($data['MasaKerja'] == "ok"){
            return " AND round(DATEDIFF('".$TglNow."',b.Tmt) / 365) = '0'";
        }else{
            return " AND round(DATEDIFF('".$TglNow."',b.Tmt) / 365) = ".$data['MasaKerja'];
        }
    }else{
        return "";
    }
...
$objPHPExcel->getProperties()->setCreator("PT ISMA")
							 ...
							 ->setTitle("Rekap Data Masa Keja")
							 ->setSubject("Rekap Data Masa Keja")
							 ->setDescription("Rekap Data Masa Keja")
							 ->setKeywords("Rekap Data Masa Keja")
							 ->setCategory("Rekap Data Masa Keja");
...
$Titles = $_GET['MasaKerja']." TAHUN";
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'REKAP DATA PERCABANG BERDASARKAN MASA KERJA '.$Titles)
            ->setCellValue('A3', 'NO')
...
header('Content-Disposition: attachment;filename="'.$Time.'-Rekap-Data-Percabang-Berdasarkan-Masa-Kerja.xls"');
