...
function FilterData($data){
    ...
    if(!empty($data['KodeDivisi']) || !empty($data['KodeSubDivisi']) || !empty($data['KodeSeksi'])){
...
function RekapData($data){
    ...
    $sql = "SELECT COUNT(Id) as Total, KodeCabang, SpkPengangkatan FROM ims_master_biodata WHERE Flag = '1' $Filter  GROUP BY KodeCabang ASC";
...
$objPHPExcel->getProperties()->setCreator("PT ISMA")
							 ...
							 ->setTitle("Rekap Data Percabang")
							 ->setSubject("Rekap Data Percabang")
							 ->setDescription("Rekap Data Percabang")
							 ->setKeywords("Rekap Data Percabang")
							 ->setCategory("Rekap Data Percabang");
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'REKAP DATA PERCABANG')
            ->setCellValue('A3', 'NO')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('REKAP DATA PERCABANG');
...
header('Content-Disposition: attachment;filename="'.$Time.'-RekapDataPercabang.xls"');
