...
function FilterData($data){
    $Filter = array();
    if(!empty($data['Agama'])){
        return " AND b.Agama = '".$data['Agama']."'";
    }else{
        return "";
    }
}

function getNamaAgama($Kode){
    if(!empty($Kode)){
        $res = $GLOBALS['db']->query("SELECT Nama FROM ims_agama WHERE Kode = '$Kode'")->fetch(PDO::FETCH_ASSOC);
        return $res['Nama'];
    }else{
        return "SEMUA AGAMA";
    }
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'REKAP DATA PERCABANG BERDASARKAN AGAMA')
            ->setCellValue('A3', 'NO')
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle($Agama);
...
header('Content-Disposition: attachment;filename="'.$Time.'-Rekap-Data-Percabang-Berdasarkan-Agama.xls"');
