...
function FilterData($data){
    ...
    if(!empty($data['Periode'])){
        return " WHERE DATE_FORMAT(a.Periode, '%Y-%m') = '".$data['Periode']."'";
    }else{
        return "";
    }
}
...
$objPHPExcel->getProperties()->setCreator("PT ISMA")
							 ...
							 ->setTitle("Rekap Data Karyawan")
							 ->setSubject("Rekap Data Karyawan")
							 ->setDescription("Rekap Data Karyawan")
							 ->setKeywords("Rekap Data Karyawan")
							 ->setCategory("Rekap Data Karyawan");
...
$Titles = strtoupper(getBulan(substr($_GET['Periode'],-2)))." ".substr($_GET['Periode'],0,4);
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A2', 'REKAP DATA PERCABANG BERDASARKAN PERIODE '.$Titles)
            ->setCellValue('A3', 'NO')
...
header('Content-Disposition: attachment;filename="'.$Time.'-Rekap-Data-Percabang-Berdasarkan-Periode.xls"');
