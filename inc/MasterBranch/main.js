...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterBranch/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Branch");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterBranch/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data);
					$("#Title").html("Ubah Data Master Branch");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'Kode', 'Nama', 'Flag'];
		...
		$("#Title").html("Tambah Data Master Branch");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#Kode").focus();
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["Kode","Nama",];
	var KetiForm = ["Kode Branch belum lengkap","Nama Branch belum lengkap"];
	var KodeError = 1;
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Branch", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterBranch/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Branch", '007', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Branch", "007", res['pesan'], 'ProsesCrud');
					StopLoad();
				}
			},
			error: function (er) {
