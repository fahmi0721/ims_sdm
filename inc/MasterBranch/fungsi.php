<?php
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT Id, Kode, Nama, Flag FROM ims_master_branch $FilterSearch";
            $query = $db->query($sql);
    ...
    function CekKode($Kode,$Id=null){
        if(!empty($Id)){
            $sql = "SELECT COUNT(Id) as tot FROM ims_master_branch WHERE Kode = '$Kode' AND Id != '$Id'";
            $query = $GLOBALS['db']->query($sql);
            $r = $query->fetch(PDO::FETCH_ASSOC);
            if($r['tot'] > 0){
                $pesan = "Kode Branch yang di entri telah di gunakan.";
                InsertLogs($pesan);
                return false;
            }else{
                return true;
            ...
            $sql = "SELECT COUNT(Id) as tot FROM ims_master_branch WHERE Kode = '$Kode'";
            $query = $GLOBALS['db']->query($sql);
            $r = $query->fetch(PDO::FETCH_ASSOC);
            if($r['tot'] > 0){
                $pesan = "Kode Branch yang di entri telah di gunakan.";
        ...
        if(is_array($data)){
            try {
                ...
                if(CekKode($data['Kode'])){
                    $sql = "INSERT INTO ims_master_branch SET Kode = :Kode, Nama = :Nama, Flag = :Flag,  TglCreate = :TglCreate,  UserCreate = :UserId";
                    ...
                    $exc->execute();
                    $msg['pesan'] = "Berhasil menambah data master branch";
                    $rMsg = "Berhasil menambah data master paket dengan nama branch <b>".$data['Nama']."</b>";
                    $msg['status'] = "sukses";
                    InsertLogs($rMsg);
                    return $msg;
                }else{
                    $msg['status'] = "gagal";
                    $msg['pesan'] = "Kode branch yang di entri telah di gunakan.";
                    return $msg;
                }
        ...
        if(is_array($data)){
            try {
                ...
                if(CekKode($data['Kode'], $data['Id'])){
                    $sql = "UPDATE ims_master_branch SET Kode = :Kode, Nama = :Nama, Flag = :Flag,  TglUpdate = :TglUpdate, UserUpdate = :UserId WHERE   Id = :Id";
                    $exc = $koneksi->prepare($sql);
                    $exc->bindParam('Kode', $data['Kode'], PDO::PARAM_STR);
                    $exc->bindParam('Nama', $data['Nama'], PDO::PARAM_STR);
                    $exc->bindParam('Flag', $data['Flag'], PDO::PARAM_STR);
                    ...
                    $exc->execute();
                    $msg['pesan'] = "Berhasil mengubah data master branch</b>";
                    $rMsg = "Berhasil mengubah data master bank dengan nama branch <b>".$data['Nama']."</b>";
                    $msg['status'] = "sukses";
                    InsertLogs($rMsg);
                    return $msg;
                }else{
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                $sql = "DELETE FROM ims_master_branch WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
                $exc->execute();
                $msg['pesan'] = "Berhasil menghapus data master branch </b>";
                $rMsg = "Berhasil menghapus data master branch dengan nama branch <b>".$rs['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
             } catch (PDOException $e) {
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_master_branch WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
