...
function getUsia(){
	$.ajax({
		type : "POST",
		url : "inc/RekapDataUsia/proses.php?proses=LoadData",
		data : "rule=Usia",
		beforeSend : function(){
			StartLoad();
		},
		success: function(r){
			...
			var opt = "<option value=''>Pilih Usia</option>";
			if(parseInt(res['Row']) > 0 ){
				for(var i=0; i < res['Data'].length; i++){
					var iData = res['Data'][i];
					opt += "<option value='" + iData['Usia'] + "'>" + iData['Usia']+" Tahun</option>";
				}
			}
			$("#Usia").html(opt);
...
function LoadData(page) {
	$("#ShowData").hide();
	var Usia = $("#Usia").val() == undefined ? "" : $("#Usia").val();
	var Pages = btoa(getUrl('page'));
	$.ajax({
		type: "POST",
		url: "inc/RekapDataUsia/proses.php?proses=DetailData",
		data: "Usia=" + Usia,
		beforeSend: function () {
			StartLoad();
		},
