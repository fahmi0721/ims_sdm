<?php
   ...
   function Usia(){
        $r = array();
        $TglNow = date("Y-m-d");
        $sql = "SELECT round(DATEDIFF('".$TglNow."',TglLahir) / 365) as Usia FROM ims_master_tenaga_kerja WHERE Flag = '1' GROUP BY round(DATEDIFF('".$TglNow."',TglLahir) / 365) ORDER BY  round(DATEDIFF('".$TglNow."',TglLahir) / 365) DESC";
        $query = $GLOBALS['db']->query($sql);
