...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterCabang/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Cabang");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterCabang/proses.php?proses=ShowData",
				data : "Id="+Id,
				beforeSend : function(data){
					StartLoad();
				},
				success: function(data){
					$("#Title").html("Ubah Data Master Cabang");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'NamaCabang', 'Flag'];
					for(var i=0; i < iForm.length; i++){
						if(iForm[i] == "Flag"){
							$("#Flag" + iForm[i]).prop("checked", true);
						}else{
		...
		$("#Title").html("Tambah Data Master Cabang");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#NamaCabang").focus();
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["NamaCabang",];
	var KetiForm = ["Nama Cabang belum lengkap"];
	var KodeError = 1;
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Cabang", "001", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterCabang/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
