   ...
   function FilterData($data){
    ...
    function TenagaKerjaDetail($data,$Dir){
        if($data['From'] == "Agama"){
            ...
            $sql = "SELECT a.NoKtp, a.Biodata, a.SpkPengangkatan FROM ims_master_biodata a INNER JOIN ims_master_tenaga_kerja b ON a.NoKtp = b.NoKtp WHERE b.Flag = '1' $Filter GROUP BY a.NoKtp ORDER BY KodeCabang ASC";
        }elseif($data['From'] == "Penempatan"){
            unset($data['From']);
            $Filter = FilterData($data);
            $sql = "SELECT NoKtp, Biodata, SpkPengangkatan FROM ims_master_biodata WHERE Flag = '1' AND (SpkKeluar IS NULL OR SpkKeluar = '') $Filter ORDER BY KodeCabang ASC";
        }elseif($data['From'] == "JenisKelamin"){
            $datas['b.JenisKelamin'] = $data['JenisKelamin'];
            $datas['a.KodeCabang'] = $data['KodeCabang'];
            $Filter = FilterData($datas);
            $sql = "SELECT a.NoKtp, a.Biodata, a.SpkPengangkatan FROM ims_master_biodata a INNER JOIN ims_master_tenaga_kerja b ON a.NoKtp = b.NoKtp WHERE b.Flag = '1' $Filter GROUP BY a.NoKtp ORDER BY a.KodeCabang ASC";
        }elseif($data['From'] == "PendidikanNonFormal"){
            $datas['b.KodeMaster'] = $data['KodeMaster'];
            $datas['a.KodeCabang'] = $data['KodeCabang'];
            $Filter = FilterData($datas);
            $sql = "SELECT a.NoKtp, a.Biodata, a.SpkPengangkatan FROM ims_master_biodata a INNER JOIN ims_pendidikan_nonformal b ON a.NoKtp = b.NoKtp WHERE a.Flag = '1' AND (a.SpkKeluar IS NULL OR a.SpkKeluar = '') $Filter GROUP BY a.NoKtp ORDER BY KodeCabang ASC";
        }elseif($data['From'] == "PendidikanFormal"){
            $datas['b.KodeMaster'] = $data['KodeMaster'];
            $datas['b.KodeSubMaster'] = $data['KodeSubMaster'];
            $datas['a.KodeCabang'] = $data['KodeCabang'];
            $Filter = FilterData($datas);
            $sql = "SELECT a.NoKtp, a.Biodata, a.SpkPengangkatan FROM ims_master_biodata a INNER JOIN ims_pendidikan_formal b ON a.NoKtp = b.NoKtp WHERE a.Flag = '1' AND (a.SpkKeluar IS NULL OR a.SpkKeluar = '') $Filter GROUP BY a.NoKtp ORDER BY KodeCabang ASC";
        }elseif($data['From'] == "Usia"){
            $datas['Usia'] = $data['Usia'];
            $TglNow = date("Y-m-d");
            $Filter = empty($datas['Usia']) ? "" : " AND round(DATEDIFF('".$TglNow."',b.TglLahir) / 365) = ".$datas['Usia'];
            $Filter .= empty($data['KodeCabang']) ? "" : " AND a.KodeCabang = '".$data['KodeCabang']."'";
            $sql = "SELECT a.NoKtp, a.Biodata, a.SpkPengangkatan FROM ims_master_biodata a INNER JOIN ims_master_tenaga_kerja b ON a.NoKtp = b.NoKtp WHERE b.Flag = '1' $Filter GROUP BY a.NoKtp ORDER BY a.KodeCabang ASC";
            
        }elseif($data['From'] == "MasaKerja"){
            $TglNow = date("Y-m-d");
            if($data['MasaKerja'] == 0){
                $Filter = " AND round(DATEDIFF('".$TglNow."',b.Tmt) / 365) = 0";
            }elseif($data['MasaKerja'] == ""){
                $Filter = "";
            }else{
                $Filter = " AND round(DATEDIFF('".$TglNow."',b.Tmt) / 365) = ".$data['MasaKerja'];
            }
