<?php
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT Id, Kode, Nama, Flag FROM ims_master_paket $FilterSearch";
            $query = $db->query($sql);
    ...
    function MaxId(){
        $sql = "SELECT MAX(RIGHT(Kode,2)) as Kode FROM ims_master_paket ORDER BY TglCreate DESC";
    ...
    function TambahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $Kode = MaxId();
                $sql = "INSERT INTO ims_master_paket SET Nama = :Nama, Kode = :Kode, Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId";
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil menambah data master paket";
                $rMsg = "Berhasil menambah data master paket dengan nama paket <b>".$data['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function UbahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $data['UserId'] = $_SESSION['Id'];
                $sql = "UPDATE ims_master_paket SET Nama = :Nama, Flag = :Flag,  TglUpdate = :TglUpdate WHERE   Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Nama', $data['Nama'], PDO::PARAM_STR);
                $exc->bindParam('Flag', $data['Flag'], PDO::PARAM_STR);
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil mengubah data master paket</b>";
                $rMsg = "Berhasil mengubah data master paket dengan nama paket <b>".$data['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
            } catch (PDOException $e) {
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                $sql = "DELETE FROM ims_master_paket WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
                $exc->execute();
                $msg['pesan'] = "Berhasil menghapus data master paket </b>";
                $rMsg = "Berhasil menghapus data master paket dengan nama paket <b>".$rs['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
             } catch (PDOException $e) {
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_master_paket WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
