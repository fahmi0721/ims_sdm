...
function SearchForm() {
	...
	$('.select-unit-kerja').on("change", function (e) {
		Clear();
		getTenagaKerja();
	});
}
...
function getUnitKerja(){
	$.ajax({
		type : "POST",
		url : "inc/DataTenagaKerjaKeluar/proses.php?proses=LoadData",
		data : "rule=UnitKerja",
		beforeSend : function(){
			StartLoad();
		},
...
function LoadData(page) {
	...
	var NoKtp = $("#NoKtp").val() == undefined ? "" : $("#NoKtp").val();
	console.log(KodeCabang);
	$.ajax({
		type: "POST",
		url: "inc/DataTenagaKerjaKeluar/proses.php?proses=DetailData",
		data: "RowPage=" + RowPage + "&Page=" + page+"&KodeCabang="+KodeCabang+"&NoKtp="+NoKtp,
		beforeSend: function () {
			StartLoad();
		},
							...
							html += "<div class='panel-body'>";
								html += "<div class='box-tk-utama'>";
									...
									html += "<div class='box-caption'>";
										...
										html += "<label class='label bg-teal'  data-toggle='tooltip' title='Jabatan/Seksi'><i class='fa fa-tag'></i> " + iData['Seksi']+"</label><br>";
										if(iData['Flag'] == "0"){
											html += "<label class='label bg-red'  data-toggle='tooltip' title='Status'><i class='fa fa-time'></i>Tidak Aktif</label>";
										}else{
											html += "<label class='label bg-green'  data-toggle='tooltip' title='Status'><i class='fa fa-checked'></i>Aktif</label>";
										}
									html += "</div>";
								html += "</div>";
							html += "</div>";
...
function Clear(){
	$("#Title").html("Pencraian Cepat Tenaga Kerja Keluar");
