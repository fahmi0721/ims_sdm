<?php
    ...
    function Filter($str){
        if(empty($str)){
            ...
            return "WHERE (Kode LIKE '%".$str."%'  OR NamaSeksi LIKE '%".$str."%') ";
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT Id, Kode, NamaSeksi, Flag FROM ims_master_seksi $FilterSearch";
            $query = $db->query($sql);
            ...
            if($JumRow > 0){
                ...
                while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
                    ...
                    $row['No'] = $no;
                    $row['Seksi'] = $res['NamaSeksi']."<br><small><b>Kode Seksi/Jabatan : ".$res['Kode']."</b></small>";
    ...
    function MaxId(){
        $sql = "SELECT MAX(Kode) as Kode FROM ims_master_seksi ORDER BY TglCreate DESC";
    ...
    function TambahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $Kode = MaxId();
                $sql = "INSERT INTO ims_master_seksi SET NamaSeksi = :NamaSeksi, Kode = :Kode,  Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('NamaSeksi', $data['NamaSeksi'], PDO::PARAM_STR);
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil menambah data master seksi";
                $rMsg = "Berhasil menambah data master seksi dengan nama cabang <b>".$data['NamaSeksi']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function UbahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $data['UserId'] = $_SESSION['Id'];
                $sql = "UPDATE ims_master_seksi SET NamaSeksi = :NamaSeksi,  Flag = :Flag,  TglUpdate = :TglUpdate WHERE   Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('NamaSeksi', $data['NamaSeksi'], PDO::PARAM_STR);
                $exc->bindParam('Flag', $data['Flag'], PDO::PARAM_STR);
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil mengubah data master seksi</b>";
                $rMsg = "Berhasil mengubah data master seksi dengan nama seksi <b>".$data['NamaSeksi']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
            } catch (PDOException $e) {
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                $sql = "DELETE FROM ims_master_seksi WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
                $exc->execute();
                $msg['pesan'] = "Berhasil menghapus data master seksi";
                $rMsg = "Berhasil menghapus data master seksi dengan nama seksi <b>".$rs['NamaSeksi']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
             } catch (PDOException $e) {
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_master_seksi WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
