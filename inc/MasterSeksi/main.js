...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterSeksi/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Seksi");
	...
	if(Id){
		if(Status == "ubah"){	
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterSeksi/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data)
					$("#Title").html("Ubah Data Master Seksi");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#BtnControl").hide();
					$("#aksi").val("update");
					var iForm = ['Id',"NamaSeksi"];
		...
		$("#Title").html("Tambah Data Master Seksi");
		$("#FormData").show();
		$("#BtnControl").hide();
		$("#DetailData").hide();
		$("#proses").html("");
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["NamaSeksi"];
	var KetiForm = ["Nama Seksi belum lengkap"];
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Seksi", "003", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterSeksi/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Seksi", '002', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Seksi", "002", res['pesan'], 'proses');
					Clear();
					StopLoad();
				}
			},
