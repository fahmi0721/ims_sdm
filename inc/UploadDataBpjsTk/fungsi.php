<?php
        ...
        try {
            ...
            foreach ($objWorksheet->getRowIterator() as $row) {
                ...
                if($sheet_start2 > 2){
                    ...
                    $res['NoKpj'] = $objPHPExcel->getActiveSheet()->getCell("C$sheet_start2")->getValue(); 
                    $res['Thn'] = $objPHPExcel->getActiveSheet()->getCell("D$sheet_start2")->getValue(); 
                    $res['Bln'] = $objPHPExcel->getActiveSheet()->getCell("E$sheet_start2")->getValue(); 
                    $res['Tgl'] = $objPHPExcel->getActiveSheet()->getCell("F$sheet_start2")->getValue(); 
                    $res['Flag'] = $objPHPExcel->getActiveSheet()->getCell("G$sheet_start2")->getValue(); 
                    
                    if($res['NoKtp'] != "" && $res['NoKpj'] != "" && $res['Thn'] != "" && $res['Bln'] != "" && $res['Tgl'] != "" && $res['Flag'] != ""){
                        $dt[] = $res;
                    }
    ...
    function CekNoKtp($NoKtp){
        ...
        if(strlen($NoKtp) != 16){
            ...
            if($r['tot'] > 0){
                ...
                $error = "data dengan no ktp ".$NoKtp." gagal di proses. data ini belum ada dalam Sitsem";
                $data['error'] = $error;
                $data['msg'] = "gagal";
                return $data;
            }
            ...
            foreach($data as $key => $r){
                ...
                if($CekNoKtp['msg'] == "sukses"){
                    try {
                        ...
                        $stmt->bindParam("NoKpj", $r['NoKpj'],PDO::PARAM_STR);
                        ...
                        $succes[] = "data BPJS TK dengan no ktp ".$r['NoKtp']." berhasil diupload";

                    } catch (PDOException $e) {
                        $error[] = "data BPJS TK dengan no ktp ".$r['NoKtp']." gagal diuplaod. Pesan error : ".$e->getMessage();
                        continue;
                    }
                }
            }
    ...
    function TambahMasterBiodata($NoKtp){
        try {
            $r = $GLOBALS['db']->query("SELECT * FROM ims_bpjs_tk WHERE NoKtp = '$NoKtp' ORDER BY Flag DESC, TglDaftar DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
            $rs = json_encode($r);
            $rs = base64_encode($rs);
            ...
            $res['status'] = "sukses";
            $res['pesan'] = "BPJS TK berhasil diupdate ke master biodata denga No KTP ".$NoKtp;
            InsertLogs($res['pesan']);
            return true;
        } catch (PDOException $e) {
            return $e->getMessage();
