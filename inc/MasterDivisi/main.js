...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterDivisi/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Divisi");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterDivisi/proses.php?proses=ShowData",
				data : "Id="+Id,
				beforeSend : function(data){
					StartLoad();
				},
				success: function(data){
					$("#Title").html("Ubah Data Master Divisi");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#BtnControl").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'NamaCabang', 'NamaDivisi', 'Flag'];
		...
		$("#Title").html("Tambah Data Master Divisi");
		$("#FormData").show();
		$("#BtnControl").hide();
		$("#DetailData").hide();
		$("#proses").html("");
...
function Validasi(){
	...
	var iForm = ["NamaDivisi"];
	var KetiForm = [,"Nama Divisi belum lengkap"];
	var KodeError = 1;
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Divisi", "002", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterDivisi/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Divisi", '002', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Divisi", "002", res['pesan'], 'proses');
					Clear();
					StopLoad();
				}
			},
