...
function Export(){
	...
	window.open("Export/RekapDataPercabang.php?" + iData, "MsgWindow", "width=200,height=100");
}
...
function SearchForm() {
	...
	$('.select-seksi').select2({
		...
		ballowClear: true,
		theme: "bootstrap",
		placeholder: 'Pilih Seksi/Jabatan',
	});
...
function getBranch(){
	$.ajax({
		type : "POST",
		url : "inc/RekapDataPercabang/proses.php?proses=LoadData",
		data : "rule=Branch",
		beforeSend : function(){
			StartLoad();
		},
	...
	$.ajax({
		...
		success: function(r){
			...
			if(parseInt(res['Row']) > 0 ){
				for(var i=0; i < res['Data'].length; i++){
					var iData = res['Data'][i];
					opt += "<option value='" + iData['Kode']+"'>"+iData['NamaDivisi']+"</option>";
				}
			}
			$("#KodeDivisi").html(opt);
...
function getSubDivisi() {
	$.ajax({
		...
		success: function (r) {
			var res = JSON.parse(r);
			var opt = "<option value=''>Pilih Sub Divisi</option>";
			if (parseInt(res['Row']) > 0) {
				for (var i = 0; i < res['Data'].length; i++) {
					var iData = res['Data'][i];
					opt += "<option value='" + iData['Kode'] + "'>" + iData['NamaSubDivisi'] + "</option>";
				}
				$("#KodeSubDivisi").html(opt);
			}
...
function getSeksi() {
	$.ajax({
		...
		success: function (r) {
			var res = JSON.parse(r);
			var opt = "<option value=''>Pilih Seksi / Jabatan</option>";
			if (parseInt(res['Row']) > 0) {
				for (var i = 0; i < res['Data'].length; i++) {
					var iData = res['Data'][i];
					opt += "<option value='" + iData['Kode'] + "'>" + iData['NamaSeksi'] + "</option>";
				}
				$("#KodeSeksi").html(opt);
			}
			StopLoad();
...
function LoadData(page) {
	$("#ShowData").hide();
	var KodeBranch = $("#KodeBranch").val() == undefined ? "" : $("#KodeBranch").val();
	var KodeDivisi = $("#KodeDivisi").val() == undefined ? "" : $("#KodeDivisi").val();
	var KodeSubDivisi = $("#KodeSubDivisi").val() == undefined ? "" : $("#KodeSubDivisi").val();
	var KodeSeksi = $("#KodeSeksi").val() == undefined ? "" : $("#KodeSeksi").val();
	var Pages = btoa(getUrl('page'));
	$.ajax({
		type: "POST",
		url: "inc/RekapDataPercabang/proses.php?proses=DetailData",
		data: "KodeDivisi=" + KodeDivisi + "&KodeSubDivisi=" + KodeSubDivisi+"&KodeSeksi="+KodeSeksi+"&KodeBranch="+KodeBranch,
		beforeSend: function () {
			StartLoad();
		},
...
function Clear(){
	$("#Title").html("Berdasarkan Penempatan");
