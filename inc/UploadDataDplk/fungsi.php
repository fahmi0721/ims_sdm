<?php
        ...
        try {
            ...
            foreach ($objWorksheet->getRowIterator() as $row) {
                ...
                if($sheet_start2 > 2){
                    ...
                    $res['Cif'] = $objPHPExcel->getActiveSheet()->getCell("C$sheet_start2")->getValue(); 
                    $res['NoAccount'] = $objPHPExcel->getActiveSheet()->getCell("D$sheet_start2")->getValue(); 
                    ...
                    if($res['NoKtp'] != "" && $res['Cif'] != "" && $res['NoAccount'] != "" && $res['Thn'] != "" && $res['Bln'] != "" && $res['Tgl'] != ""){
                        $dt[] = $res;
                    }
            ...
            foreach($data as $key => $r){
                ...
                if($CekNoKtp['msg'] == "sukses"){
                    try {
                        ...
                        $stmt->bindParam("Cif", $r['Cif'],PDO::PARAM_STR);
                        $stmt->bindParam("NoAccount", $r['NoAccount'],PDO::PARAM_STR);
                        ...
                        $succes[] = "data DPLK dengan no ktp ".$r['NoKtp']." berhasil diupload";

                    } catch (PDOException $e) {
                        $error[] = "data DPLK dengan no ktp ".$r['NoKtp']." gagal diuplaod. Pesan error : ".$e->getMessage();
                        continue;
                    }
                }
            }
    ...
    function TambahMasterBiodata($NoKtp){
        try {
            $r = $GLOBALS['db']->query("SELECT * FROM ims_master_dplk WHERE NoKtp = '$NoKtp' ORDER BY Flag DESC, TglDaftar DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
            $rs = json_encode($r);
            $rs = base64_encode($rs);
            ...
            $res['status'] = "sukses";
            $res['pesan'] = "Data DPLK berhasil diupdate ke master biodata denga No KTP ".$NoKtp;
            InsertLogs($res['pesan']);
            return true;
        } catch (PDOException $e) {
            return $e->getMessage();
