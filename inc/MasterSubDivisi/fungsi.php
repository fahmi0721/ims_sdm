<?php
    ...
    function Filter($str){
        if(empty($str)){
            ...
            return "WHERE (Kode LIKE '%".$str."%' OR NamaSubDivisi LIKE '%".$str."%') ";
        }
    }
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT Id, Kode, NamaSubDivisi, Flag  FROM ims_master_subdivisi  $FilterSearch";
            $query = $db->query($sql);
            ...
            if($JumRow > 0){
                ...
                while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
                    ...
                    $row['No'] = $no;
                    $row['SubDivisi'] = $res['NamaSubDivisi']."<br><small><b>Kode Sub Divisi : ".$res['Kode']."</b></small>";
    ...
    function MaxId(){
        $sql = "SELECT MAX(Kode) as Kode FROM ims_master_subdivisi ORDER BY TglCreate DESC";
    ...
    function TambahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $Kode = MaxId();
                $sql = "INSERT INTO ims_master_subdivisi SET NamaSubDivisi = :NamaSubDivisi, Kode = :Kode, Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('NamaSubDivisi', $data['NamaSubDivisi'], PDO::PARAM_STR);
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil menambah data master sub divisi";
                $rMsg = "Berhasil menambah data master divisi dengan nama cabang <b>".$data['NamaSubDivisi']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function UbahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $data['UserId'] = $_SESSION['Id'];
                $sql = "UPDATE ims_master_subdivisi SET NamaSubDivisi = :NamaSubDivisi,  Flag = :Flag,  TglUpdate = :TglUpdate WHERE   Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('NamaSubDivisi', $data['NamaSubDivisi'], PDO::PARAM_STR);
                $exc->bindParam('Flag', $data['Flag'], PDO::PARAM_STR);
                ...
                $rMsg = "Berhasil mengubah data master divisi dengan nama divisi <b>".$data['NamaSubDivisi']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
            } catch (PDOException $e) {
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                $sql = "DELETE FROM ims_master_subdivisi WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
                $exc->execute();
                $msg['pesan'] = "Berhasil menghapus data master sub divisi";
                $rMsg = "Berhasil menghapus data master cabang dengan nama divisi <b>".$rs['NamaSubDivisi']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
             } catch (PDOException $e) {
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_master_subdivisi WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
