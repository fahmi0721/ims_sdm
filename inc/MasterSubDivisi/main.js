...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterSubDivisi/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Sub Divisi");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterSubDivisi/proses.php?proses=ShowData",
				data : "Id="+Id,
				beforeSend : function(data){
					StartLoad();
				},
				success: function(data){
					$("#Title").html("Ubah Data Master Sub Divisi");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#BtnControl").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'Flag',"NamaSubDivisi"];
		...
		$("#Title").html("Tambah Data Master Sub Divisi");
		$("#FormData").show();
		$("#BtnControl").hide();
		$("#DetailData").hide();
		$("#proses").html("");
...
function Validasi(){
	...
	var iForm = ["NamaSubDivisi"];
	var KetiForm = ["Nama Sub Divisi belum lengkap"];
	
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Sub Divisi", "003", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterSubDivisi/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Sub Divisi", '002', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Sub Divisi", "002", res['pesan'], 'proses');
					Clear();
					StopLoad();
				}
			},
