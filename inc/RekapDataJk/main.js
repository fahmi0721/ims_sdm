...
function Export(){
	...
	window.open("Export/RekapDataAgama.php?" + iData, "MsgWindow", "width=200,height=100");
}
...
function getJk(){
	var opt = "<option value=''>Pilih Agama</option>";
	opt += "<option value='L'>Laki - Laki</option>";
	opt += "<option value='P'>Perempuan</option>";
	$("#Jk").html(opt);
...
function LoadData(page) {
	$("#ShowData").hide();
	var Jk = $("#Jk").val() == undefined ? "" : $("#Jk").val();
	var Pages = btoa(getUrl('page'));
	$.ajax({
		type: "POST",
		url: "inc/RekapDataJk/proses.php?proses=DetailData",
		data: "Jk=" + Jk,
		beforeSend: function () {
			StartLoad();
		},
...
function Clear(){
	$("#Title").html("Berdasarkan Jenis Kelamin");
