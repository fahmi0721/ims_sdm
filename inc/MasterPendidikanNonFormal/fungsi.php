<?php
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT Id, Kode, Nama, Keterangan, Flag FROM ims_master_pendidikan_nonformal $FilterSearch";
            $query = $db->query($sql);
    ...
    function getKode(){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT MAX(Kode) as Kode FROM ims_master_pendidikan_nonformal";
    ...
    function TambahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $Kode = getKode();
                $sql = "INSERT INTO ims_master_pendidikan_nonformal SET Kode = :Kode, Nama = :Nama, Keterangan = :Keterangan, Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId";
                ...
                $rMsg = "Berhasil menambah data master pendidikan non formal dengan nama <b>".$data['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function UbahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $data['UserId'] = $_SESSION['Id'];
                $sql = "UPDATE ims_master_pendidikan_nonformal SET Nama = :Nama, Flag = :Flag, Keterangan = :Keterangan,  TglUpdate = :TglUpdate WHERE   Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Nama', $data['Nama'], PDO::PARAM_STR);
                $exc->bindParam('Flag', $data['Flag'], PDO::PARAM_STR);
                $exc->bindParam('Keterangan', $data['Keterangan'], PDO::PARAM_STR);
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil mengubah data master pedidikan non formal</b>";
                $rMsg = "Berhasil mengubah data master pedidikan non formal dengan nama pedidikan <b>".$data['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                $sql = "DELETE FROM ims_master_pendidikan_nonformal WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
                $exc->execute();
                $msg['pesan'] = "Berhasil menghapus data master pendidikan non formal </b>";
                $rMsg = "Berhasil menghapus data master pendidikan non formal dengan nama pendidikan <b>".$rs['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
             } catch (PDOException $e) {
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_master_pendidikan_nonformal WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
