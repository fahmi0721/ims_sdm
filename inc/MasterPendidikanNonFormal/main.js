...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterPendidikanNonFormal/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Pendidikan Non Formal");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterPendidikanNonFormal/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data);
					$("#Title").html("Ubah Data Master Pendidikan Non Formal");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#aksi").val("update");
					$('#BtnControl').hide();
					var iForm = ['Id','Nama', 'Flag','Keterangan'];
		...
		$("#Title").html("Tambah Data Master Pendidikan Non Formal");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#Kode").focus();
...
function Validasi(){
	...
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Pendidikan Non Formal", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterPendidikanNonFormal/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Pendidikan Non Formal", '007', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Pendidikan Non Formal", "007", res['pesan'], 'ProsesCrud');
					StopLoad();
				}
			},
			error: function (er) {
