...
function LoadTK(){
	$.ajax({
		type : 'POST',
		url : 'inc/NoRekening/proses.php?proses=LodaData',
...
function LoadBank() {
	$.ajax({
		...
		success: function (res) {
			...
			for (var i = 0; i < r.length; i++) {
				var iData = r[i];
				html += "<option value='" + iData['Kode'] + "'>" + iData['Kode'] + " - " + iData['Nama'] + "</option>";
			}
			$("#KodeBank").html(html);
		},
		error: function (er) {
...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/NoRekening/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Nomor Rekening");
	...
	$(".select-agama").val(null);
	$(".select-no-ktp").trigger('change');
	$(".select-kode-bank").trigger('change');
	ClearModal();
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/NoRekening/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data);
					$("#Title").html("Ubah Data Nomor Rekening");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'NoKtp', 'KodeBank', 'NoRek', 'Flag'];
			...
			var Tipe = spli[1];
			if(Tipe != "pdf"){
				$("#proses_del").html("<center><img class='img-responsive' src='img/FileRekening/"+File+"'></center>");
			}
			$(".modal-footer").hide();
		...
		$("#Title").html("Tambah Data Nomor Rekening");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#Kode").focus();
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["NoKtp","KodeBank","NoRek"];
	var KetiForm = ["Tenaga Kerja belum lengkap", "Bank belum lengkap","Nomor Rekening belum lengkap"];
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { 
				if(iForm[i] == "NoKtp"){
					$(".select-no-ktp").select2("focus");
				}else if(iForm[i] == "KodeBank"){
					$(".select-kode-bank").select2("focus");
				}
				Customerror("Nomor Rekening", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; 
			}
		}
	}
}
...
function SubmitData(){
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/NoRekening/proses.php?proses=Crud",
			processData: false,
			contentType: false,
			chace: false,
			data: data,
			...
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Nomor Rekening", '007', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Nomor Rekening", "007", res['pesan'], 'ProsesCrud');
					StopLoad();
				}
			},
			error: function (er) {
