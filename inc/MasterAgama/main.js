...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterAgama/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Agama");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterAgama/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data);
					$("#Title").html("Ubah Data Master Agama");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#aksi").val("update");
		...
		$("#Title").html("Tambah Data Master Agama");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#Nama").focus();
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["Nama",];
	var KetiForm = ["Nama Agama belum lengkap"];
	var KodeError = 1;
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { Customerror("Master Agama", "005", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; }
		}
	}
}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterAgama/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Agama", '005', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Agama", "005", res['pesan'], 'proses');
					Clear();
					StopLoad();
				}
			},
