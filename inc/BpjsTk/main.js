...
function getTK(){
	$.ajax({
		type : "POST",
		url : "inc/BpjsTk/proses.php?proses=LoadData"		,
...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/BpjsTk/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data BPJS Ketenagakerjaan");
	$("#ProsesCrud").html("");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/BpjsTk/proses.php?proses=ShowData",
				data : "Id="+Id,
				beforeSend : function(data){
					StartLoad();
				},
				success: function(data){
					$("#Title").html("Ubah Data BPJS Ketenagakerjaan");
					$("#FormData").show();
					$(".box-tools").hide();
					$("#DetailData").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'NoKtp', 'NoKpj', 'TglDaftar','Flag','Keterangan'];
			...
			$("#proses_del").html("<center><img class='img-responsive' src='File/BpjsTk/"+Id+"'></center>");
			$(".modal-footer").hide();
		...
		$("#Title").html("Tambah Data BPJS Ketenagakerjaan");
		$("#FormData").show();
		$("#DetailData").hide();
		$(".box-tools").hide();
		$("#proses").html("");
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ['NoKtp', 'NoKpj'];
	var KetiForm = ["No KTP belum lengkap", "No KPJ belum lengkap" ];
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "" || $("#" + iForm[i]).val() == null) { 
				...
				Customerror("BPJS Ketenagakerjaan", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; 
			}
		}
	}
...
function SubmitData(){
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/BpjsTk/proses.php?proses=Crud",
			processData : false,
			contentType : false,
			chace : false,
			data: data,
			...
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("BPJS Ketenagakerjaan", '007', res['pesan'],'proses');
					LoadData();
					StopLoad();
					scrolltop();
				}else{
					Customerror("BPJS Ketenagakerjaan", "007", res['pesan'], 'ProsesCrud');
					scrolltop();
					StopLoad();
				}
			},
