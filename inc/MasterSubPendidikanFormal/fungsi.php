<?php
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT Kode, Id, Kode,Nama, Flag FROM ims_master_sub_pendidikan_formal  $FilterSearch";
            $query = $db->query($sql);
            ...
            if($JumRow > 0){
                ...
                while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
                    ...
                    $row['No'] = $no;
                    $row['Jurusan'] = $res['Nama']."<br><small><b>Kode : ".$res['Kode']."</b></small>";
    ...
    function getKode(){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT MAX(Kode) as Kode FROM ims_master_sub_pendidikan_formal";
    ...
    function TambahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $Kode = getKode();
                $sql = "INSERT INTO ims_master_sub_pendidikan_formal SET Kode = :Kode, Nama = :Nama, Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId";
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil menambah data master sub pendidikan formal";
                $rMsg = "Berhasil menambah data master sub pendidikan formal dengan nama <b>".$data['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function UbahData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $data['UserId'] = $_SESSION['Id'];
                $sql = "UPDATE ims_master_sub_pendidikan_formal SET Nama = :Nama, Flag = :Flag,  TglUpdate = :TglUpdate WHERE   Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Nama', $data['Nama'], PDO::PARAM_STR);
                $exc->bindParam('Flag', $data['Flag'], PDO::PARAM_STR);
                ...
                $exc->execute();
                $msg['pesan'] = "Berhasil mengubah data master sub pedidikan formal</b>";
                $rMsg = "Berhasil mengubah data master sub pedidikan formal dengan nama pedidikan <b>".$data['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                $sql = "DELETE FROM ims_master_sub_pendidikan_formal WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
                $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
                $exc->execute();
                $msg['pesan'] = "Berhasil menghapus data master sub pendidikan formal </b>";
                $rMsg = "Berhasil menghapus data master sub pendidikan formal dengan nama pendidikan <b>".$rs['Nama']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
                return $msg;
             } catch (PDOException $e) {
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_master_sub_pendidikan_formal WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
