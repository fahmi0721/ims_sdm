...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/MasterSubPendidikanFormal/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Master Sub Pendidikan Formal");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/MasterSubPendidikanFormal/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data);
					$("#Title").html("Ubah Data Master Sub Pendidikan Formal");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#aksi").val("update");
					$('#BtnControl').hide();
		...
		$("#Title").html("Tambah Data Master Sub Pendidikan Formal");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["Nama"];
	var KetiForm = ["Nama Sub Jurusan belum lengkap"];
	var KodeError = 1;
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "" || $("#" + iForm[i]).val() == null) { 
				Customerror("Master Sub Pendidikan Formal", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); 
				return false; 
			}
		}
	}
...
function SubmitData(){
	...
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/MasterSubPendidikanFormal/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Master Sub Pendidikan Formal", '007', res['pesan'],'proses');
					LoadData();
					StopLoad();
				}else{
					Customerror("Master Sub Pendidikan Formal", "007", res['pesan'], 'ProsesCrud');
					StopLoad();
				}
			},
			error: function (er) {
