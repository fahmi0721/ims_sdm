...
function Export(){
	...
	window.open("Export/RekapDataMasaKerja.php?" + iData, "MsgWindow", "width=200,height=100");
}
...
function getMasKerja(){
	$.ajax({
		type : "POST",
		url: "inc/RekapDataMasaKerja/proses.php?proses=LoadData",
		data: "rule=MasaKerja",
		beforeSend : function(){
			StartLoad();
		},
		success: function(r){
			...
			var opt = "<option value=''>Pilih Masa Kerja</option>";
			if(parseInt(res['Row']) > 0 ){
				for(var i=0; i < res['Data'].length; i++){
					var iData = res['Data'][i];
					var Masa = iData['MasaKerja'] <= 0 ? "< 1 " : iData['MasaKerja'];
					opt += "<option value='" + iData['MasaKerja'] + "'>" +Masa +" Tahun</option>";
				}
			}
			$("#MasaKerja").html(opt);
...
function LoadData(page) {
	$("#ShowData").hide();
	var MasaKerja = $("#MasaKerja").val() == undefined ? "" : $("#MasaKerja").val();
	var Pages = btoa(getUrl('page'));
	$.ajax({
		type: "POST",
		url: "inc/RekapDataMasaKerja/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Berdasarkan Masa Kerja");
