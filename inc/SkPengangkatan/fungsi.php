<?php
    ...
    function Filter($str){
        $src = array();
        foreach($str as $key => $vl){
            if(!empty($vl)){
                $src[] = $key." = '".$vl."'";
            }
        }
        return !empty($src) ? "AND ".implode("AND ",$src) : "";
    }
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT a.File, a.Id, a.NoKtp, a.NoDokumen, a.KodeCabang, a.KodeDivisi, a.KodeSubDivisi, a.KodeSeksi, a.TanggalSelesai, a.TanggalMulai, a.Keterangan, b.Nama FROM ims_sk_pengangkatan a INNER JOIN ims_master_tenaga_kerja b ON a.NoKtp = b.NoKtp WHERE Kategori = '0' $FilterSearch";
            $query = $db->query($sql);
            ...
            if($JumRow > 0){
                while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
                    ...
                    if(empty($res['File'])){
                        ...
                        if($extensi == "pdf"){
                             $aksi = "<a href='File/SkPengangkatan/".$res['File']."' class='btn btn-xs btn-success' data-toggle='tooltip' title='Lihat Dokumen' target='blank'><i class='fa ".$icon."'></i></a> ";
    ...
    function getNameDivisi($Kode){
        ...
        $query = $GLOBALS['db']->query($sql);
        $r = $query->fetch(PDO::FETCH_ASSOC);
        return empty($r['Nama']) ? "Kosng" : $r['Nama'];
    }
    ...
    function UpdateMasterBiodata($NoKtp){
        try {
            ...
            if(empty($rt)){
                ...
                $rt['NamaBracnh'] = getNameUnitKeraja($rt['KodeBranch']);
            ...
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    if( $Validasi['msg'] == "sukses"){
                        ...
                        $exc->execute();
                        $msg['pesan'] = "Berhasil menambah data SK PKWT";
                        $rMsg = "Berhasil menambah data SK PKWT dengan no ktp <b>".$data['NoKtp']."</b>";
                        $msg['status'] = "sukses";
                        InsertLogs($rMsg);
        ...
        if(is_array($data)){
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    $exc->bindParam('Id', $data['Id'], PDO::PARAM_STR);
                    $exc->execute();
                    $msg['pesan'] = "Berhasil mengubah data SK PKWT";
                    $rMsg = "Berhasil mengubah data SK PKWT dengan no ktp <b>".$data['NoKtp']."</b>";
                    $msg['status'] = "sukses";
                    InsertLogs($rMsg);
                    UpdateMasterBiodata($data['NoKtp']);
                    return $msg;
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                ...
                $msg['pesan'] = "Berhasil menghapus data SK PKWT </b>";
                $rMsg = "Berhasil menghapus data SK PKWT dengan No KTP <b>".$rs['NoKtp']."</b>";
                $msg['status'] = "sukses";
                InsertLogs($rMsg);
