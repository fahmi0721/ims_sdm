<?php
    ...
    function Filter($str){
        if(empty($str)){
            ...
            return "WHERE (a.NoJkn LIKE '%".$str."%' OR b.Nama LIKE '%".$str."%')";
        }
    }
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT a.Id, a.File, a.NoJkn, a.StatusKepesertaan, a.TglDaftar, a.Flag, a.NoKtp, b.Nama FROM ims_bpjs_kesehatan a INNER JOIN ims_master_tenaga_kerja b ON a.NoKtp = b.NoKtp $FilterSearch";
            $query = $db->query($sql);
            ...
            if($JumRow > 0){
                while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
                    if(!empty($res['File'])){
                        ...
                        if($extensi == "pdf"){
                            $aksi = "<a target='_blank' href='File/BpjsKesehatan/".$res['File']."' class='btn btn-xs btn-success' data-toggle='tooltip' title='Lihat Dokumen' ><i class='fa fa-file-pdf-o'></i></a>";
                        }else{
                    ...
                    $row['NoJkn'] = $res['NoJkn'];
                    $row['Status'] = $StatusKepesertaan[$res['StatusKepesertaan']];
    ...
    function UpdateMasterBiodata($NoKtp){
        try {
            $qr = $GLOBALS['db']->query("SELECT * FROM ims_bpjs_kesehatan WHERE NoKtp = '$NoKtp' ORDER BY Flag DESC, TglCreate DESC LIMIT 1");
            ...
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    if($Img['status'] == "sukses"){
                        $sql = "INSERT INTO ims_bpjs_kesehatan SET  NoKtp = :NoKtp, NoJkn = :NoJkn,  StatusKepesertaan = :StatusKepesertaan, TglDaftar = :TglDaftar, `File` = :Files,  Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId"; 
                        $exc = $koneksi->prepare($sql);
                        $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                        $exc->bindParam('NoJkn', $data['NoJkn'], PDO::PARAM_STR);
                        $exc->bindParam('StatusKepesertaan', $data['StatusKepesertaan'], PDO::PARAM_STR);
                        ...
                        $exc->execute();
                        $msg['pesan'] = "Berhasil menambah data BPJS Kesehatan";
                        $rMsg = "Berhasil menambah data BPJS Kesehatan dengan No KTP <b>".$data['NoKtp']."</b>";
                        $msg['status'] = "sukses";
                        InsertLogs($rMsg);
            ...
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    if($Img['status'] == "sukses"){
                        ...
                        $sql = "UPDATE ims_bpjs_kesehatan SET  NoKtp = :NoKtp, NoJkn = :NoJkn,  StatusKepesertaan = :StatusKepesertaan, TglDaftar = :TglDaftar, `File` = :Files,  Flag = :Flag,  TglUpdate = :TglUpdate WHERE  Id = :Id"; 
                        $exc = $koneksi->prepare($sql);
                        $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                        $exc->bindParam('NoJkn', $data['NoJkn'], PDO::PARAM_STR);
                        $exc->bindParam('StatusKepesertaan', $data['StatusKepesertaan'], PDO::PARAM_STR);
                        ...
                        $exc->execute();
                        $msg['pesan'] = "Berhasil mengubah data BPJS Kesehatan";
                        $rMsg = "Berhasil mengubah data BPJS Kesehatan dengan No KTP <b>".$data['NoKtp']."</b>";
                        $msg['status'] = "sukses";
                        InsertLogs($rMsg);
                        UpdateMasterBiodata($data['NoKtp']);
                        return $msg;
                    ...
                    $sql = "UPDATE ims_bpjs_kesehatan SET  NoKtp = :NoKtp, NoJkn = :NoJkn,  StatusKepesertaan = :StatusKepesertaan, TglDaftar = :TglDaftar,  Flag = :Flag,  TglUpdate = :TglUpdate WHERE  Id = :Id"; 
                    $exc = $koneksi->prepare($sql);
                    $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                    $exc->bindParam('NoJkn', $data['NoJkn'], PDO::PARAM_STR);
                    $exc->bindParam('StatusKepesertaan', $data['StatusKepesertaan'], PDO::PARAM_STR);
    ...
    function HapusData($data){
        ...
        if(is_array($data)){
            try {
                $rs = ShowData($data['Id']);
                HapusFile($data['Id'], $data['Dir']);
                $sql = "DELETE FROM ims_bpjs_kesehatan WHERE Id = :Id";
                $exc = $koneksi->prepare($sql);
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM ims_bpjs_kesehatan WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
