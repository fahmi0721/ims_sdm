...
function getTK(){
	$.ajax({
		type : "POST",
		url : "inc/PendidikanNonFormal/proses.php?proses=LoadData"		,
...
function getSertifikasi() {
	$.ajax({
		type: "POST",
		url: "inc/PendidikanNonFormal/proses.php?proses=LoadData",
		data: "rule=MasterPendidikanNonFormal",
		beforeSend: function () {
			StartLoad();
		},
		success: function (res) {
			var r = JSON.parse(res);
			var html = "<option value=''>Pilih Sertifikasi</option>";
			for (var i = 0; i < r['data'].length; i++) {
				var iData = r['data'][i];
...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/PendidikanNonFormal/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data Pendidikan Non Formal");
	$("#ProsesCrud").html("");
	...
	$(".select-sertifikasi").val(null);
	$(".select-no-ktp").trigger("change");
	$(".select-sertifikasi").trigger("change");
	ClearModal();
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/PendidikanNonFormal/proses.php?proses=ShowData",
				data : "Id="+Id,
				beforeSend : function(data){
					StartLoad();
				},
				success: function(data){
					$("#Title").html("Ubah Data Pendidikan Non Formal");
					$("#FormData").show();
					$(".box-tools").hide();
					$("#DetailData").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'NoKtp', 'KodeMaster', 'Dari', 'Sampai','Flag','Keterangan'];
			...
			$("#proses_del").html("<center><img class='img-responsive' src='File/PendidikanNonFormal/"+Id+"'></center>");
			$(".modal-footer").hide();
		...
		$("#Title").html("Tambah Data Pendidikan Non Formal");
		$("#FormData").show();
		$("#DetailData").hide();
		$(".box-tools").hide();
		$("#proses").html("");
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ['NoKtp', 'KodeMaster', 'Dari', 'Sampai'];
...
function SubmitData(){
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/PendidikanNonFormal/proses.php?proses=Crud",
			processData : false,
			contentType : false,
			chace : false,
			data: data,
			...
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Pendidikan Non Formal", '007', res['pesan'],'proses');
					LoadData();
					StopLoad();
					scrolltop();
				}else{
					Customerror("Pendidikan Non Formal", "007", res['pesan'], 'ProsesCrud');
					scrolltop();
					StopLoad();
				}
			},
