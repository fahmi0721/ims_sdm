<?php
    ...
    function DetailData($data){
        ...
        if(is_array($data)){
            ...
            $sql = "SELECT a.Keterangan, a.NoKtp, a.Flag, a.Id, a.Dari, a.Sampai, b.Nama as NamaMaster, a.File, c.Nama as TK FROM ims_pendidikan_nonformal a INNER JOIN ims_master_pendidikan_nonformal b ON a.KodeMaster = b.Kode  INNER JOIN ims_master_tenaga_kerja c ON a.NoKtp = c.NoKtp $FilterSearch";
            $query = $db->query($sql);
            ...
            if($JumRow > 0){
                while ($res = $query->fetch(PDO::FETCH_ASSOC)) { 
                    ...
                    $row['Pendidikan'] = $res['NamaMaster'];
                    $row['Berlaku'] = "Dari : ".tgl_indo($res['Dari'])."<br>Sampai : ".tgl_indo($res['Sampai']);
            ...
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    if($Img['status'] == "sukses"){
                        $sql = "INSERT INTO ims_pendidikan_nonformal SET  NoKtp = :NoKtp, KodeMaster = :KodeMaster,  Dari = :Dari, Sampai = :Sampai, `File` = :Files, Keterangan = :Keterangan, Flag = :Flag,  TglCreate = :TglCreate,  UserId = :UserId"; 
                    ...
                    $sql = "INSERT INTO ims_pendidikan_nonformal SET  NoKtp = :NoKtp, KodeMaster = :KodeMaster, Dari = :Dari, Sampai = :Sampai,  Flag = :Flag, Keterangan = :Keterangan,  TglCreate = :TglCreate,  UserId = :UserId"; 
                    $exc = $koneksi->prepare($sql);
                    $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                    $exc->bindParam('KodeMaster', $data['KodeMaster'], PDO::PARAM_STR);
                    $exc->bindParam('Dari', $data['Dari'], PDO::PARAM_STR);
            ...
            try {
                ...
                if(isset($data['File']) && !empty($data['File']['name'])){
                    ...
                    if($Img['status'] == "sukses"){
                        ...
                        $sql = "UPDATE ims_pendidikan_nonformal SET NoKtp = :NoKtp, KodeMaster = :KodeMaster, Dari = :Dari, Sampai = :Sampai, `File` = :Files, Flag = :Flag, Keterangan = :Keterangan,  TglUpdate = :TglUpdate WHERE  Id = :Id"; 
                        $exc = $koneksi->prepare($sql);
                        $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                        $exc->bindParam('KodeMaster', $data['KodeMaster'], PDO::PARAM_STR);
                        $exc->bindParam('Dari', $data['Dari'], PDO::PARAM_STR);
                    ...
                    $sql = "UPDATE ims_pendidikan_nonformal SET  NoKtp = :NoKtp, KodeMaster = :KodeMaster, Dari = :Dari, Sampai = :Sampai,  Flag = :Flag, Keterangan = :Keterangan,  TglUpdate = :TglUpdate WHERE Id = :Id"; 
                    $exc = $koneksi->prepare($sql);
                    $exc->bindParam('NoKtp', $data['NoKtp'], PDO::PARAM_STR);
                    $exc->bindParam('KodeMaster', $data['KodeMaster'], PDO::PARAM_STR);
                    $exc->bindParam('Dari', $data['Dari'], PDO::PARAM_STR);
