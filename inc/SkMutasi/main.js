...
function LoadTK(){
	$.ajax({
		type : 'POST',
		url: 'inc/SkMutasi/proses.php?proses=LoadData',
...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/SkMutasi/proses.php?proses=DetailData",
...
function Clear(){
	$("#Title").html("Tampil Data SK Mutasi");
	...
	if(Id){
		if(Status == "ubah"){
			$.ajax({
				type : "POST",
				dataType : "json",
				url: "inc/SkMutasi/proses.php?proses=ShowData",
				data : "Id="+Id,
				...
				success: function(data){
					console.log(data);
					$("#Title").html("Ubah Data SK Mutasi");
					$("#FormData").show();
					$("#DetailData").hide();
		...
		$("#Title").html("Tambah Data SK Mutasi");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#NoKtp").focus();
...
function Validasi(){
	var aksi = $("#aksi").val();
	var iForm = ["NoKtp", "No", "NoDokumen", "KodeBranch", "KodeCabang", "KodeDivisi", "KodeSubDivisi", "KodeSeksi","TanggalMulai"];
	...
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "") { 
				...
				Customerror("SK Mutasi", "007", KetiForm[i], 'ProsesCrud'); $("#" + iForm[i]).focus(); scrolltop(); return false; 
			}
		}
	}
}
...
function SubmitData(){
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/SkMutasi/proses.php?proses=Crud",
			processData: false,
			contentType: false,
			chace: false,
			data: data,
